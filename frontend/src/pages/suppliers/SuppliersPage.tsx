  ...
  return (
    <div className="space-y-4">
      <div className="page-header">
        ...
        <button className="btn-primary" onClick={() => setShowNew(true)}><Plus className="w-4 h-4" /> New Supplier</button>
      </div>
      <div className="card">
            ...
            <tbody>
              {data?.results?.map((s: { id: number; code: string; name: string; contact_name: string; email: string; payment_terms: string; lead_time_days: number; status: string }) => (
                <tr key={s.id} className="cursor-pointer" onClick={() => nav(`/suppliers/${s.id}`)}>
                  ...
                  <td className="text-sm text-gray-500">{s.contact_name}{s.email ? ` · ${s.email}` : ''}</td>
                  <td><span className="badge badge-blue">{s.payment_terms?.replace(/_/g,' ')}</span></td>
                  <td>{s.lead_time_days}d</td>
                  <td><StatusBadge status={s.status} /></td>
                  <td onClick={e => e.stopPropagation()}>
                    <button onClick={() => nav(`/suppliers/${s.id}`)} className="btn btn-secondary btn-sm"><Eye className="w-3 h-3" /></button>
                  </td>
                </tr>
              ))}
            </tbody>
